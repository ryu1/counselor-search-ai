...
セッションメモリが正しく機能するか確認するテストがあります。このテストは以下のフローを検証します：

1. 会話履歴を蓄積（相談内容 → 相談方法 → 駅 → 性別 → 日時）
2. 各ステップでセッションメモリが正しく会話を記憶しているか確認
3. 最終的にすべての条件を含む確認画面が表示されるか確認

```bash
npm test -- --grep "セッションメモリ"  # セッションメモリテストのみ実行
```
